...
def ml_loop():
    ...
    while True:
        ...
        if vectors_y!=0:
         for j in range(len(scene_info.bricks)):
             if scene_info.bricks[j][1]-ball_y>0 :
                if ball_x+((scene_info.bricks[j][1]-ball_y)/vectors_y)*vectors_x<scene_info.bricks[j][0]+8 or ball_x+((scene_info.bricks[j][1]-ball_y)/vectors_y)*vectors_x>scene_info.bricks[j][0]-8: 
         ...
         if len(scene_info.hard_bricks)!=0:
             for j in range(len(scene_info.hard_bricks)):
                if scene_info.hard_bricks[j][1]-ball_y>0 :
                    if ball_x+((scene_info.hard_bricks[j][1]-ball_y)/vectors_y)*vectors_x<scene_info.hard_bricks[j][0]+8 or ball_x+((scene_info.hard_bricks[j][1]-ball_y)/vectors_y)*vectors_x>scene_info.hard_bricks[j][0]-8: 
        ...
        if vectors_y!=0:
         for j in range(len(scene_info.bricks)):
            if scene_info.bricks[j][1]-ball_y>0 and tempbrick_x!=scene_info.bricks[j][0]:
                if tempbrick_x+((scene_info.bricks[j][1]-tempbrick_y)/vectors_y)*tempvectors_x<scene_info.bricks[j][0]+8 or tempbrick_x+((scene_info.bricks[j][1]-tempbrick_y)/vectors_y)*tempvectors_x>scene_info.bricks[j][0]-8:
                 if ball_willbe_x<scene_info.bricks[j][0] and tempvectors_x<0:
                     ball_willbe_x=scene_info.bricks[j][0]+(scene_info.bricks[j][0]-ball_willbe_x)
                     tempbrick_y=scene_info.bricks[j][1]
                     tempbrick_x=scene_info.bricks[j][0]
                     tempvectors_x=tempvectors_x*-1
                     j=100
                 elif ball_willbe_x>scene_info.bricks[j][0] and tempvectors_x>0:
                     ball_willbe_x=scene_info.bricks[j][0]-(ball_willbe_x-scene_info.bricks[j][0])
                     tempbrick_y=scene_info.bricks[j][1]
                     tempbrick_x=scene_info.bricks[j][0]
                     tempvectors_x=tempvectors_x*-1
        ...
        if vectors_y!=0:
         if len(scene_info.hard_bricks)!=0:
            for j in range(len(scene_info.hard_bricks)):
              if scene_info.hard_bricks[j][1]-ball_y>0 and tempbrick_x!=scene_info.hard_bricks[j][0]:
                if tempbrick_x+((scene_info.hard_bricks[j][1]-tempbrick_y)/vectors_y)*tempvectors_x<scene_info.hard_bricks[j][0]+8 or tempbrick_x+((scene_info.hard_bricks[j][1]-tempbrick_y)/vectors_y)*tempvectors_x>scene_info.hard_bricks[j][0]-8 :
                    if ball_willbe_x<scene_info.hard_bricks[j][0] and tempvectors_x<0:
                        ball_willbe_x=scene_info.hard_bricks[j][0]+(scene_info.hard_bricks[j][0]-ball_willbe_x)
                        tempbrick_y=scene_info.hard_bricks[j][1]
                        tempbrick_x=scene_info.hard_bricks[j][0]
                        tempvectors_x=tempvectors_x*-1
                        j=100
                    elif ball_willbe_x>scene_info.hard_bricks[j][0] and tempvectors_x>0:
                        ball_willbe_x=scene_info.hard_bricks[j][0]-(ball_willbe_x-scene_info.hard_bricks[j][0])
                        tempbrick_y=scene_info.hard_bricks[j][1]
                        tempbrick_x=scene_info.hard_bricks[j][0]
                        tempvectors_x=tempvectors_x*-1
